export function sendNotification(type, text) {
  ...
  component.className = `relative flex items-center  z-[100] text-white text-sm font-bold px-4 py-3 rounded-md opacity-0 transform transition-all duration-500 mb-1`;
  component.innerHTML = `${alerts[type].icon}<p>${text}</p>`;
  component.style.backgroundColor = alerts[type].color;
  component.style.color = alerts[type].color == "#FFEB3B" ? "black" : "white";
